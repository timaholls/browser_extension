...
async function setupProxy(profileKey) {
    ...
    // Админ подключается к Squid
    console.log(`🔧 Админ подключается к порту ${localPort}`);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'switchProfile') {
        ...
        chrome.storage.local.set({userProfile: request.profileKey}, () => {
            setupProxy(request.profileKey).then(() => {
                sendResponse({
                    success: true,
                    message: `Переключен на ${AUTH_CONFIG.userAccounts[request.profileKey].name}`
                });
            });
        });
        return true;
    ...
    if (request.action === 'saveProfile') {
        ...
        chrome.storage.local.set({userProfile: request.profileKey}, () => {
            ...
            sendResponse({
                success: true,
                message: `Профиль "${AUTH_CONFIG.userAccounts[request.profileKey].name}" сохранен и подключен`
            });
        });
        return true;
    }
